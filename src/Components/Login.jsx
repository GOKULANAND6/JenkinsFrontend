...
function Login() {
      ...
      const fetchUsers = async () => {
        ...
        try {
          const response = await axios.get(
            "http://localhost:8003/signup/all"
          );
    ...
    const handleSubmit = (e) => {
      ...
      const user = registeredUsers.find(
        (user) => user.signup_email === email && user.signup_password === password
      );
   
      if (user) {
        // localStorage.setItem(user.sign_email, user.sign_password);
        navigate("/homepage2");
      } else {
            ...
            <div className="w-full space-y-6 text-gray-600 sm:max-w-md">
                <div className="text-center">
                    <div className="mt-5 space-y-2">
                        <h3 className="text-gray-800 text-2xl font-bold sm:text-3xl" role='login-heading'>Log in to your account</h3>
                        <p className="font-medium text-indigo-600 hover:text-indigo-500"  role='p-heading'>Don't have an account? <a href="javascript:void(0)" className="font-medium text-indigo-600 hover:text-indigo-500">Sign up</a></p>
                    </div>
                </div>
                <div className="bg-white shadow p-4 py-6 space-y-8 sm:p-6 sm:rounded-lg">
                    ...
                    <div className="relative">
                        ...
                        <p className="inline-block w-fit text-sm bg-white px-2 absolute -top-2 inset-x-0 mx-auto" role='p2-heading'>Or continue with</p>
                    </div>
                    <form
                    ...
                    >
                        <div>
                            <label className="font-medium" role='email'>
                                Email
                            </label>
                            <input
                                type="email"
                        ...
                        <div>
                            <label className="font-medium" role='password'>
                                Password
                            </label>
                            <input
                                type="password"
