...
function Signup() {
    ...
    const [inputData, setInputData] = useState({
      ...
      let handleSubmit = (e) => {
        ...
        if (result === true) {
          axios
            .post("http://localhost:8003/signup", inputData)
            .then((res) => {
    ...
    const validateValues = (inputData) => {
        if (inputData.signup_name.length === 0) {
          alert("Please enter the Name !!! ");
          return false;
        } else if (inputData.signup_email.length ===0) {
          alert("Please enter the Email !!!");
          return false;
        } else if (inputData.signup_password.length === 0) {
    ...
    <div className="max-w-sm w-full text-gray-600 space-y-8">
        <div className="text-center">
            <div className="mt-5 space-y-2">
                <h3 className="text-gray-800 text-2xl font-bold sm:text-3xl" role='heading'>Signup to your account</h3>
            </div>
        </div>
        <form
        ...
        >
            <div>
                <label className="font-medium" role='namelabel'>
                    Name
                </label>
                <input
                    type="text"
                    ...
                    onChange={(e) =>
                        setInputData({ ...inputData, signup_name: e.target.value })
                      }
                />
            </div>

            <div>
                <label className="font-medium" role='emaillabel'>
                    Email
                </label>
                <input
                    type="email"
                    ...
                    className="w-full mt-2 px-3 py-2 text-gray-500 bg-transparent outline-none border focus:border-indigo-600 shadow-sm rounded-lg"
                    onChange={(e) =>
                        setInputData({ ...inputData, signup_email: e.target.value })
                      }
                />
            </div>

            <div>
                <label className="font-medium" role='passlabel'>
                    Password
                </label>
                <input
                    type="password"
                    ...
                    className="w-full mt-2 px-3 py-2 text-gray-500 bg-transparent outline-none border focus:border-indigo-600 shadow-sm rounded-lg"
                    onChange={(e) =>
                        setInputData({ ...inputData, signup_password: e.target.value })
                      }
                />
            </div>
        ...
        <div className="relative">
            ...
            <p className="inline-block w-fit text-sm bg-white px-2 absolute -top-2 inset-x-0 mx-auto" role='p-heading'>Or continue with</p>
        </div>
